...
Database::Database(const char * filenamefmt, size_t cachesize, int64_t loadidx) :
  ...
  this->active = buffer;
  fileblocks = WITE::getFileSize(buffer) / BLOCKSIZE;
  ...
  WITE::Thread::spawnThread(WITE::CallbackFactory<void>::make<Database>(this, &Database::primeThreadEntry));
}
...
void Database::gracefulStop() {
  ...
  if(WITE::Thread::getCurrentTid() == pt_tid) {
    if(was == 1) {
      pt_syncToDisk();
      //LOG("db synced\n");
...
void Database::getRecurse(Entry e, uint8_t* out, uint64_t* starts, uint64_t* lens, size_t count,
  ...
  case branch:
    ...
    while(*regionIdx < count && nextBlockIdx < le.children.subblockCount) {
      ...
      nextBlockIdx = WITE::min(nextBlockIdx + 1, (starts[*regionIdx] - *offsetOfE) / BLOCKDATASIZE);
    }
...
void Database::getRaw(Entry e, size_t offset, size_t size, uint8_t* out, uint64_t frame) {
  ...
  if(allocTab[e].nextWriteFrame <= frame) {
    ...
    while(tlogPoint != NULL_ENTRY) {
      ...
      case update:
        start = WITE::max(offset, log.offset);
        end = WITE::min(offset + size, log.offset + log.size);//FIXME log.offset is wrong
        if(start < end)
...
Database::Entry Database::allocate(size_t size, type t) {//allocation is handled by master thread, submit request and wait
...
void WITE::Database::registerType(type t, typeHandles funcs) {
  ::Database::registerType(t, funcs);
}
...
WITE::Database::typeHandles* WITE::Database::getHandlesOfType(type t) {
  return ::Database::getHandlesOfType(t);
}
...
WITE::Object** Database::getObjectInstanceFor(Entry e) {
...
void Database::pt_rethinkLayout(size_t requiredNewEntries) {
  ...
  newRecordCount = WITE::min(idealFree + atCount - pt_freeSpace, atCount);
  newCacheCount = (primeRamSize - newRecordCount * sizeof(allocationTableEntry) -
    WITE::max(pt_writenBytesLastFrame, pt_writenBytesThisFrame, 65536)) / (2 * sizeof(cacheEntry));
  ...
  TIME(
    for(;pt_rethinkCache_asyncVars.entryScanIdx < atCount;pt_rethinkCache_asyncVars.entryScanIdx++) {
      ...
      if(ate.allocationState == unallocated)
        ...
        score = WITE::max(ate.readsLastFrame, ate.readsThisFrame) +
...
Database::Entry Database::pt_allocate(size_t size, Entry* map) {//evergreen model
  ...
  for(i = j = 0;i < numBlocks;i++) {
    ...
    if(i < numDataEntries) {
      ...
      *le = {currentFrame, update, sizeof(state_t), offsetof(loadedEntry, header.state), allocationTableEntry::OBJECT_NULL, j};
      ...
      statePutter.data.children.subblockCount = subblockCount = WITE::min(orphans, MAXBLOCKPOINTERS);
      ...
      *le = {currentFrame, update, sizeof(_entry) + sizeof(blocklistData::subblockCount) + subblockCount * sizeof(Entry), 0, allocationTableEntry::OBJECT_NULL, j};
      ...
      statePutter.data.children.subblockCount = subblockCount = WITE::min(unboundBranches, MAXBLOCKPOINTERS - !(i == numBlocks - 1));
  ...
  if(map) memcpy(map, entries, WITE::min(ALLOCATION_MAP_SIZE, numDataEntries * sizeof(entries[0])));
  j = entries[0];
  return j;
}
...
bool Database::pt_adoptTlog(threadResource::transactionalBacklog_t* src) {
  ...
  if(ele.type == del) {
    ...
    do {
      ...
      case state_t::data:
        size = WITE::min(ele.size + ele.offset - offset, BLOCKSIZE);
...
std::unique_ptr<WITE::Database> WITE::Database::makeDatabase(const char * filenamefmt, size_t cachesize, int64_t loadidx) {
  return std::make_unique<::Database>(filenamefmt, cachesize, loadidx);
};

std::unique_ptr<WITE::Database> WITE::Database::makeDatabase(size_t cachesize) {
  return std::make_unique<::Database>(cachesize);
};
