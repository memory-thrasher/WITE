...
class export_def WITE::GPU {//these are created by init and may be present as args in callbacks
 public:
  ~GPU() = default;
...
template<class T, class D = std::default_delete<T>> class export_def WITE::GPUResource {//mostly thread safe, dirty read possible, most gpu stuff is kept on main thread
 private:
