...
std::unique_ptr<VertexBuffer[VERTEX_BUFFERS]> makeBuffersFor(GPU* gpu) {
  VertexBuffer* ret = malloc(sizeof(VertexBuffer) * VERTEX_BUFFERS);
  for(size_t i = 0;i < VERTEX_BUFFERS;i++)
    new(&ret[i].verts)BackedBuffer(gpu, vulkan_singleton.vramGrabSize/VERTEX_BUFFERS, VK_BUFFER_USAGE_VERTEX_STORAGE_BIT);
  return std::unique_ptr<VertexBuffer[VERTEX_BUFFERS]>(ret);
}

uint32_t Mesh::put(void* out, uint32_t offset, uint32_t maxSize, GPU* gpu) {
  size_t camCount = 0, camIdx;//TODO better
  std::shared_ptr<Object> object, mostSignificantChild;
  Camera* camera, *mostSignificantCamera;
  ...
  for (auto window = gpu->windows.begin();window != end;window++) {
	...
	relativeResolution = camera->approxScreenArea(object->getTrans()->transform(source->getBbox(&tempBox1), &tempBox2));
